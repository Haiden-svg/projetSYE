...
class Main:
    ...
    ##############################################
    # Task system #
    dico={"t1": [], "t2": [t1], "t3": [], "t4": [t1,t2,t3]}
    ts = Tasksystem([t1, t2, t3, t4], dico)
    ##############################################
    # instruction #
    failed, passed =ts.bernsteinMap(t4)
    for task in failed:
        print("failed=",task.name)
    for task in passed:
        print("passed=",task.name)
        failed1, passed1 =ts.bernsteinMap(t2)
    for task in failed1:
        print("failed1=",task.name)
    for task in passed1:
        print("passed1=",task.name)
