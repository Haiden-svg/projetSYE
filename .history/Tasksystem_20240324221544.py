...
##############################################
    ...
    def getDependecies(self,task):
        return self.dico[task.name]
    def getDependencie(self,task):
        
        road = [[]]
        passed,failed = self.bernsteinMap(task)
        if failed == [] :
            print("no parallelism with",task.name)
            return road
        road.append(passed)
        for tasked in passed:
            print("parallelism between", task.name, "and", tasked.name)
        for task2 in failed:
            if task2 in self.dico[task.name]:
                print("go to the next task")
                self.getDependencie(task2)

    def getDependencieTEST(self,task):
        road = [[]]
        after =[[]]
        before = [[]]
        index1,index2=0,0
        ...
        for i in range(100):
            for task2 in tasks:
                if task.bernstein(task2):
                    before[i].append(task2)
                    tasks.remove(task2)
                elif task.bernstein(task2) == False and task2 not in self.dico[task.name] and task2 not in after:
                    after[index1].append(task2)
                    tasks.remove(task2)
                else:
                    before[i+1].append(task2)
                    tasks.remove(task2)                    
##############################################   
    ...
    def runseq(self):
        ...
        while x==0:
            for task in tasks:
                dependencies = self.getDependecies(task)
    ...
    def bernsteinMap(self,task):
        ...
        for task2 in self.tasks:
            if task == task2:
                continue
            if task.bernstein(task2):
                bernstein_passed.append(task2)
            else:
                bernstein_failed.append(task2)
        return bernstein_failed, bernstein_passed
