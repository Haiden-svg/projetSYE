...
import random
##############################################
#args: tks: liste de taks, dico: dictionnary of dependencies
class Tasksystem:
        #constructor#
...
##############################################
    ...
    def draw(self):
        G = nx.DiGraph()
        for task in self.tasks:
            G.add_node(task.name) # Add a node for each task
        for task in self.tasks:
            for t in self.dico[task.name]:
                if t == []: # If the task has no dependencies,
                    continue     
                G.add_edge(t.name,task.name) # Add an edge between the task and its dependencies
