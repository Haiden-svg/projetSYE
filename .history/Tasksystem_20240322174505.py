...
##############################################    
    ...
    def bernsteinIntoTasks(self):
        ...
        tasks_to_test2=self.tasks.copy()
        for task in tasks_to_test:
            for task2 in tasks_to_test2:
                if task == task2:
                    print("No Bernstein between",task.name,"and",task2.name, "because they are the same task so removed from the list of tasks to test")
                    tasks_to_test2.remove(task2)
                    continue
                if task.bernstein(task2):
                    print("Bernstein between",task.name,"and",task2.name)
                else:
                    print("No Bernstein between",task.name,"and",task2.name)
            print("End of the test for",task.name,"removed from the list of tasks to test")
            tasks_to_test.remove(task)        
